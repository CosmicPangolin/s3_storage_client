...
class S3Storage {
  ...
  Future<String> getBucketRegion(String? bucket) async {
    ...
    if (bucket == null) {
      throw StorageInvalidBucketNameError('No region set. Bucket name is required.');
    }
  ...
  Future presignedPostPolicy(PostPolicy postPolicy) async {
    ...
    postPolicy.policy['conditions'].add(['eq', r'$x-amz-credential', accessKey + '/' + getScope(region, date)]);
